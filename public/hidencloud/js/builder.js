...
export function bindBuilder(root) {
    ...
    const startupCb = $("#b-startup");
    const debugCb = $("#b-debug");
    function update() {
    ...
    buildBtn.addEventListener("click", async () => {
        ...
        try {
            const res = await api.build({ buildName, startup, debug });
            const done = document.createElement("div");
            done.className = "build-line ok";
            done.textContent = `✓ Build "${buildName}" ready — ${res.c2Url}`;
            ...
            if (res.cargoContents) {
                ...
                ca.href = URL.createObjectURL(new Blob([res.cargoContents], { type: "text/plain" }));
            ...
            if (res.persistenceContents) {
                const pa = document.createElement("a");
                pa.href = URL.createObjectURL(new Blob([res.persistenceContents], { type: "text/plain" }));
